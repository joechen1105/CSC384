...
class MonteCarloAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for action in legalActions:
            ...
            SuccessorStateVal.append(
                float(self.wins[successorState]) / float(self.plays[successorState]))
        ...
        * Remember to limit the depth of the search only in the expansion phase!
        ...
        if(len(self.plays) == 0):
            ...
            while(1):
                ...
                if any(s in level_nodes for s in self.plays.keys()):
                ...
                # if not any(s in level_nodes for s in self.plays.keys()):
                if(level_nodes[0] not in self.plays.keys()):
                    # expansion
                    for s in level_nodes:
                        self.plays[s] = 0
                        self.wins[s] = 0
                    ...
                    # roll out
                    value = self.simulation(cur_node, self.depth, agentIndex)
                    for s in visited_nodes:
                        self.plays[s] += 1
                        self.wins[s] += value
                    break
                else:
                    ...
                    ucb1Dict = {}
                    for s in level_nodes:
                        ucb1Dict[s] = self.UCB1(cur_node, s)
                    cur_node = max(ucb1Dict.iterkeys(), key=(lambda key: ucb1Dict[key]))
                    visited_nodes.append(cur_node)
                if(agentIndex < (gameState.getNumAgents() - 1)):
                    agentIndex += 1
                else:
